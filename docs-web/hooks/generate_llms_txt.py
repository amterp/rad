...
def _embedded_body(embedded_dir, filepath):
    ...
    if path.exists():
        return _strip_generated_banner(path.read_text(encoding="utf-8"))
    return None


def _strip_generated_banner(text):
    """Drop the leading `<!-- GENERATED by tools/... -->` banner line that
    gen-docs-embed prepends to every core/embedded_docs/ file. Mirrors
    rts.StripGeneratedBanner (rts/generated_banner.go) - keep in sync."""
    if text.startswith("<!-- GENERATED by tools/"):
        nl = text.find("\n")
