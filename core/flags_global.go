...
func CreateAndRegisterGlobalFlags() []RslArg {
	...
	if shouldAddFlag(FLAG_HELP, FLAG_H) {
		FlagHelp = NewBoolRadArg(flagOrEmpty(FLAG_HELP), flagOrEmpty(FLAG_H), "Print usage string.", false)
		flags = append(flags, &FlagHelp)
	}

	if shouldAddFlag(FLAG_DEBUG, FLAG_D) {
		FlagDebug = NewBoolRadArg(flagOrEmpty(FLAG_DEBUG), flagOrEmpty(FLAG_D), "Enables debug output. Intended for RSL script developers.", false)
	...
	if shouldAddFlag(FLAG_COLOR, "") {
		FlagColor = NewStringRadArg(flagOrEmpty(FLAG_COLOR), flagOrEmpty(""), "mode", "Control output colorization.", "auto", &MODES, nil)
		flags = append(flags, &FlagColor)
	}

	if shouldAddFlag(FLAG_QUIET, FLAG_Q) {
		FlagQuiet = NewBoolRadArg(flagOrEmpty(FLAG_QUIET), flagOrEmpty(FLAG_Q), "Suppresses some output.", false)
	...
	if shouldAddFlag(FLAG_CONFIRM_SHELL, "") {
		FlagConfirmShellCommands = NewBoolRadArg(flagOrEmpty(FLAG_CONFIRM_SHELL), flagOrEmpty(""), "Confirm all shell commands before running them.", false)
		flags = append(flags, &FlagConfirmShellCommands)
	}

	if shouldAddFlag(FLAG_SRC, "") {
		FlagSrc = NewBoolRadArg(flagOrEmpty(FLAG_SRC), flagOrEmpty(""), "Instead of running the target script, just print it out.", false)
