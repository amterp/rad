...
This is powered by invocation logging - see Configuration (rad docs guide/config) for details.
...
Scripts can persist their own data in stashes (rad docs guide/stashes). `rad stash` lets you
...
case is minting stash IDs (rad docs guide/stashes) for new scripts.
...
your own scripts. It gets a dedicated page: Shell Completion (rad docs guide/shell-completion).
