...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.12 (rad docs migrations/v0.12) | Shell commands quote interpolated values, shell captures rebind, list args collect every positional, `split()` and `replace()` match literally |
| v0.9 (rad docs migrations/v0.9)   | `request` and `display` blocks unified into `rad`, `??` fires on null, strict `+` concatenation, `parse_epoch` unit names shortened            |
| v0.8 (rad docs migrations/v0.8)   | `get_default` removed in favor of the `??` operator                                                                                            |
| v0.7 (rad docs migrations/v0.7)   | For-loop index syntax replaced by `with loop`                                                                                                  |
...
Upgrade using whichever method you originally installed with - your package manager, `go install`, or by downloading the latest binary from the releases page (https://github.com/amterp/rad/releases). See Installation (rad docs guide/getting-started) for details.
...
**Tip: Keep invocation logging enabled**
    ...
    `rad check --from-logs` relies on Rad's invocation logging, which is enabled by default since v0.9. If you've disabled it, consider re-enabling it so it's ready when you need it. See the Configuration guide (rad docs guide/config) for details.
